...
public class CaseService
{
    ...
    public async Task SetCaseAsClosedAsync(Guid caseId, string userId)
    {
        var caseItem = await _context.CRS_Cases.FindAsync(caseId);
        if (caseItem == null)
        {
            throw new InvalidOperationException($"無法找到該案例");
        }

        caseItem.CaseCloseStatus = CaseStatus.Closed.GetDescription();
        caseItem.CaseCloseTime = DateTime.UtcNow;
        caseItem.CaseCloseCareProviderName = userId;

        await _context.SaveChangesAsync();
    }

    public async Task SetCaseAsOpenAsync(Guid caseId, string userId)
    {
        var caseItem = await _context.CRS_Cases.FindAsync(caseId);
        if (caseItem == null)
        {
        ...
        caseItem.CaseCloseStatus = CaseStatus.InProgress.GetDescription();
        caseItem.CaseCloseTime = null;
        caseItem.CaseCloseCareProviderName = userId;
